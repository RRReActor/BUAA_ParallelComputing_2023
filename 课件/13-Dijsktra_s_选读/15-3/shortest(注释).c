...
void FindMinWay(int my_rank,int group_size,int ep,int mynum)
{
	...
	for(i=0; i<nodenum;i++) //nodenum是图上节点
	{
		index = 0;
		num = M;   //无穷大
		...
		for(j=0;j<mynum;j++)
		{
			if(dist[j] < num && bdist[j]==FALSE) //j不在集合中
			{
				num = dist[j];
...
int main(int argc,char** argv)
{
	...
	*/
	...
	ep = nodenum/(group_size-1);  //ep是平均分配的图节点数量
	if(ep*group_size-ep < nodenum) ep++;  //最后一个节点少分

	if(ep*my_rank <= nodenum)   //非最后一个节点
	{
		mynum = ep;             //前面的节点
	}else if(ep*my_rank < nodenum+ep)
	{
		mynum = nodenum - ep*(my_rank-1);  //最后一个节点
	}
	else mynum = 0;
