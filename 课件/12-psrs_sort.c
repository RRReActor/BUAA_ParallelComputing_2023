...
Psrs_Main()
{
    ...
    if (size > 1)
    {
        ...
        temp1 = (int *)malloc(sizeof(int) * size * Spt);
        if (temp1 == 0)
    ...
    if (size > 1)
    {
        ...
        if (rank == 0)
        {
            ...
            MPI_Barrier(MPI_COMM_WORLD);
            quicksort(temp1, 0, size * Spt - 1);
            MPI_Barrier(MPI_COMM_WORLD);

            for (i = 1; i < size; i++)
                temp1[i] = temp1[i * (size - 1) - 1];
            /*处理器P0将这P-1个主元播送到所有处理器中，对应于算法13.5步骤（4）*/
            MPI_Bcast(temp1 + 1, sizeof(int) * (size - 1), MPI_CHAR, 0, MPI_COMM_WORLD);
            MPI_Barrier(MPI_COMM_WORLD);
        }
        else
        {
            MPI_Send(temp1, sizeof(int) * Spt, MPI_CHAR, 0, ALLTOONE_TYPE + rank, MPI_COMM_WORLD);
            MPI_Barrier(MPI_COMM_WORLD);
            MPI_Barrier(MPI_COMM_WORLD);
        ...
        /*每个处理器根据上步送来的P-1个主元把自己的n/P个数据分成P段，记为处理器Pi的第j+1段，其中i=0,…,size-1，j=0,…,size-1，对应于算法13.5步骤（5）*/
        n = mylength;
        index[0] = 0;
        i = 1;
        while ((arr[0] >= temp1[i]) && (i < size))
        {
            index[2 * i - 1] = 0;
            index[2 * i] = 0;
            i++;
        ...
        for (i = 0; i < size; i++)
        {
            ...
            if (i == rank)
            {
                for (n = 0; n < size; n++)
                    if (n != rank)
                    {
                        MPI_Send(&arr[index[2 * n]], sizeof(int) * (index[2 * n + 1] - index[2 * n]), MPI_CHAR, n, MULTI_TYPE + rank, MPI_COMM_WORLD);
                    }
            }
            else
            {
        ...
        k = 0;
        multimerge(arr1, temp1, arr, &k, size);
        MPI_Barrier(MPI_COMM_WORLD);
    }
...
/*串行多路归并算法*/
multimerge(int *data1, int *ind, int *data, int *iter, int size)
{
    int i, j, n;
    ...
    if (j > 1)
    {
        ...
        if (j % 2 == 1)
            for (i = 0; i < ind[j - 1]; i++)
                data[n++] = data1[n];
        (*iter)++;
        multimerge(data, ind, data1, iter, size);
    }
}
